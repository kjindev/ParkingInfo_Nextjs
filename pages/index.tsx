...
export default function Home() {
  ...
  const query = useQuery("parking", async () =>
    (
      await fetch(
        `http://openapi.seoul.go.kr:8088/${process.env.NEXT_PUBLIC_API_KEY}/json/GetParkInfo/1/1000/`
      )
    ).json()
  );
  ...
  const handleMapClick = (event: React.MouseEvent<HTMLElement>) => {
    ...
    if (targetElement.dataset.name !== undefined) {
      ...
      for (let i = 0; i < parkingData.length; i++) {
        if (parkingData[i].ADDR.split(" ")[0] === targetElement.dataset.name) {
  ...
  return (
    <>
      ...
      <div className="w-[100%] h-[100vh] flex justify-center items-center bg-gradient-to-r from-sky-200 to-blue-200">
        <div className="p-5 w-[90%] h-[85%] bg-white rounded-3xl drop-shadow-xl">
          <div className="flex text-5xl">서울시 공영주차장 안내</div>
          <div className="w-[100%] h-[100%] flex flex-col md:flex-row items-center justify-center">
            <div
              onClick={handleMapClick}
              ref={dataRef}
              className="w-[50%] h-[50%] drop-shadow-xl flex justify-center items-center"
            >
            ...
            <div className="overflow-x-hidden w-[40%] h-[50%]" id="scroll">
              {parkingList.map((item, index) => (
                <div key={index}>
                  <Link
                    key={item.PARKING_CODE}
                    href={{
                      pathname: `${item.PARKING_CODE}`,
                      query: {
                        id: item.PARKING_CODE,
                        name: item.PARKING_NAME,
                      },
                    }}
                    as={`${item.PARKING_CODE}`}
                  >
                    <div className="w-[90%] h-[30%] m-3 p-3 bg-white hover:bg-sky-100 drop-shadow-lg rounded-2xl">
